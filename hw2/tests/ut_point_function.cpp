...
TEST(PointTest, DistanceTest) {
    ...
    double actual_result = get_euclidean_distance(p, q);
...
TEST(PointTest, FirstQuadrantTest) {
    ...
    std::string actual_result = get_quadrant(p);
...
TEST(PointTest, CollinearVerticalTest) {
    ...
    bool actual_result = is_collinear(p, q, r);
...
TEST(PointTest, MidpointTest) {
    ...
    Point actual_result = get_midpoint(p, q);
